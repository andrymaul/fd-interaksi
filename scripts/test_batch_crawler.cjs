...
async function crawlDrug(ddinterId) {
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    try {
      ...
      return { success: true, ddinterId, drugName, formula, descLength: desc.length, hasSvg: !!svg };
    } catch (err) {
...
async function runWorker(queue) {
  while (queue.length > 0) {
    ...
    console.log(`[${item.ddinter_id}] ${item.name} -> ${res.success ? 'OK' : 'ERR: ' + res.error} (${Date.now() - t0}ms)`);
  }
}
...
async function main() {
  ...
  console.log(`Testing batch crawl with ${rows.length} drugs, concurrency = 5...`);
  const t0 = Date.now();
  const queue = [...rows];
  const workers = Array.from({ length: 5 }, () => runWorker(queue));
  await Promise.all(workers);
  console.log(`Batch of ${rows.length} finished in ${(Date.now() - t0)/1000}s!`);
}
